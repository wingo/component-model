...
class Task(Call, Supertask):
  ...
  def yield_(self, thread, cancellable, for_callback) -> EventTuple:
    if not self.wait_until(lambda: True, thread, cancellable, for_callback):
      return (EventCode.TASK_CANCELLED, 0, 0)
    else:
      return (EventCode.NONE, 0, 0)

  def wait_for_event(self, thread, wset, cancellable, for_callback) -> EventTuple:
    wset.num_waiting += 1
    if not self.wait_until(wset.has_pending_event, thread, cancellable, for_callback):
  ...
  def poll_for_event(self, thread, wset, cancellable, for_callback) -> Optional[EventTuple]:
    wset.num_waiting += 1
    if not self.wait_until(lambda: True, thread, cancellable, for_callback):
      event = (EventCode.TASK_CANCELLED, 0, 0)
    ...
    while code != CallbackCode.EXIT:
      match code:
        case CallbackCode.YIELD:
          event = task.yield_(thread, cancellable = True, for_callback = True)
        case CallbackCode.WAIT:
          wset = task.inst.table.get(si)
          trap_if(not isinstance(wset, WaitableSet))
          event = task.wait_for_event(thread, wset, cancellable = True, for_callback = True)
        case CallbackCode.POLL:
          wset = task.inst.table.get(si)
          trap_if(not isinstance(wset, WaitableSet))
          event = task.poll_for_event(thread, wset, cancellable = True, for_callback = True)
...
def canon_yield(cancellable, thread):
  trap_if(not thread.task.inst.may_leave)
  event_code,_,_ = thread.task.yield_(thread, cancellable, for_callback = False)
  match event_code:
    case EventCode.NONE:
      return [0]
...
def canon_waitable_set_wait(cancellable, mem, thread, si, ptr):
  ...
  trap_if(not isinstance(wset, WaitableSet))
  event = thread.task.wait_for_event(thread, wset, cancellable, for_callback = False)
...
def canon_waitable_set_poll(cancellable, mem, thread, si, ptr):
  ...
  wset = thread.task.inst.table.get(si)
  trap_if(not isinstance(wset, WaitableSet))
  event = thread.task.poll_for_event(thread, wset, cancellable, for_callback = False)
  return unpack_event(mem, thread, ptr, event)
