...
class Store:
  ...
  def call(self, f: FuncInst, caller: Optional[Supertask], on_start: OnStart, on_resolve: OnResolve) -> Call:
...
class Task(Call, Supertask):
  ...
  def wait_until(self, ready_func, cancellable, for_callback) -> bool:
    ...
    completed = self.thread.suspend_until(ready_and_uncontended, cancellable)
    if for_callback:
      assert(not self.inst.exclusive)
      self.inst.exclusive = True
      self.waiting_for_callback = False
  ...
  def yield_(self, cancellable, for_callback) -> EventTuple:
    if not self.wait_until(lambda:True, cancellable, for_callback):
      return (EventCode.TASK_CANCELLED, 0, 0)
    else:
      return (EventCode.NONE, 0, 0)

  def wait_for_event(self, wset, cancellable, for_callback) -> EventTuple:
    wset.num_waiting += 1
    if not self.wait_until(wset.has_pending_event, cancellable, for_callback):
  ...
  def poll_for_event(self, wset, cancellable, for_callback) -> Optional[EventTuple]:
    wset.num_waiting += 1
    if not self.wait_until(lambda:True, cancellable, for_callback):
      event = (EventCode.TASK_CANCELLED, 0, 0)
    ...
    while code != CallbackCode.EXIT:
      match code:
        case CallbackCode.YIELD:
          event = task.yield_(cancellable = True, for_callback = True)
        case CallbackCode.WAIT:
          wset = task.inst.table.get(si)
          trap_if(not isinstance(wset, WaitableSet))
          event = task.wait_for_event(wset, cancellable = True, for_callback = True)
        case CallbackCode.POLL:
          wset = task.inst.table.get(si)
          trap_if(not isinstance(wset, WaitableSet))
          event = task.poll_for_event(wset, cancellable = True, for_callback = True)
...
def canon_yield(cancellable, task):
  trap_if(not task.inst.may_leave)
  event_code,_,_ = task.yield_(cancellable, for_callback = False)
  match event_code:
    case EventCode.NONE:
      return [0]
...
def canon_waitable_set_wait(cancellable, mem, task, si, ptr):
  ...
  wset = task.inst.table.get(si)
  trap_if(not isinstance(wset, WaitableSet))
  event = task.wait_for_event(wset, cancellable, for_callback = False)
...
def canon_waitable_set_poll(cancellable, mem, task, si, ptr):
  ...
  wset = task.inst.table.get(si)
  trap_if(not isinstance(wset, WaitableSet))
  event = task.poll_for_event(wset, cancellable, for_callback = False)
  return unpack_event(mem, task, ptr, event)
